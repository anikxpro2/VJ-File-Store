...
async def incoming_gen_link(bot, message):
    ...
    if WEBSITE_URL_MODE == True:
        share_link = f"{WEBSITE_URL}?Anik_Mon={outstr}"
    else:
...
async def gen_link_s(bot, message):
    ...
    if file_type not in [enums.MessageMediaType.VIDEO, enums.MessageMediaType.AUDIO, enums.MessageMediaType.DOCUMENT, enums.MessageMediaType.TEXT]:
...
# Ask Doubt on telegram @KingVJ01
    ...
    user = await get_user(user_id)
    if WEBSITE_URL_MODE == True:
        share_link = f"{WEBSITE_URL}?Anik_Mon=BATCH-{file_id}"
    else:
